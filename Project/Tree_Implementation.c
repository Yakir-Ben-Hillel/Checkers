...
static void freeTreeAux(SingleSourceMovesTreeNode* source);
...
void freeTree(SingleSourceMovesTree* tr)
{
	if (tr->source == NULL)
		...
		freeTreeAux(tr->source);
}
static void freeTreeAux(SingleSourceMovesTreeNode* source)
{
	if (source->next_move[0] == NULL && source->next_move[1] == NULL) // A leaf
	{
		free(source->pos); // Free pointer to structure of position - the allocated memory for the col and row 
		free(source); // Free the current node - the SingleSourceMovesTreeNode allocated memory - the board, the captures and so on
	}
	else
	{	// Recursive calls 
		if (source->next_move[0] != NULL)
			freeTreeAux(source->next_move[0]);
		if (source->next_move[1] != NULL)
			freeTreeAux(source->next_move[1]);
	}
}
