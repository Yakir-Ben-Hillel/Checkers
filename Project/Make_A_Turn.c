...
static unsigned short findPlaceofBestTurn(MultipleSingleSourceMovesList *mul_list, BOOL *hasCaptured)
{
	...
	SingleSourceMovesListCell *nextPosition = NULL; // keeps the next position in case 
	BOOL first = TRUE; // flag notices first time updating the place
	while (ptr != NULL)
	{
		...
		// In case no captures - Max didn't updated
		if ((max == 0) && (nextPosition != NULL) )
		{
			if (first)
			{
				place = counter; // Updating the place to be the place of the 
				first = FALSE; // Updating the flag notices first time initializing the place variable 
			}
			else
			{
				if (rand() % 2) // Making a random choice of move in case of "no captures" case 
					place = counter;
			}
		}
		ptr = ptr->next;
