...
SingleSourceMovesListCell* FindSingleSourceOptimalMoveAux(SingleSourceMovesTreeNode *source, SingleSourceMovesList *OptimalMoveList, unsigned short count);

SingleSourceMovesList *FindSingleSourceOptimalMove(SingleSourceMovesTree *movesTree)
{ // Initialized due to security reasons
	SingleSourceMovesList *OptimalMoveList= (SingleSourceMovesList*)malloc(sizeof(SingleSourceMovesList)); 
	checkAllocation(OptimalMoveList);
	...
	{
		OptimalMoveList->head=FindSingleSourceOptimalMoveAux(movesTree->source, OptimalMoveList, countCap); // Finds the single source optimal moves
	}
...
SingleSourceMovesListCell* FindSingleSourceOptimalMoveAux(SingleSourceMovesTreeNode *source, SingleSourceMovesList *OptimalMoveList, unsigned short count)
{
	...
	SingleSourceMovesList *leftList = (SingleSourceMovesList*)malloc(sizeof(SingleSourceMovesList));
	checkAllocation(leftList);
	SingleSourceMovesList *rightList = (SingleSourceMovesList*)malloc(sizeof(SingleSourceMovesList));
	checkAllocation(rightList);
	makeEmptyList(leftList);
	makeEmptyList(rightList);
